...
CANCEL_ORDER_URL = f"{SUPABASE_URL}/rest/v1/rpc/CancelOrder"

# Initialize FastAPI Router
router = APIRouter()


def process_cancel_order_response(decrypted_data, authtokens):
    ...
    response = requests.post(CANCEL_ORDER_URL, json=payload, headers=headers)
...
async def cancel_order(request: EncryptedDataRequest, authtoken: str = Header(...)):
    """Decrypts encrypted data, extracts fields, and processes order cancellation in Supabase."""
    try:
    ...
    supabase_response = process_cancel_order_response(decrypted_data, authtokens["details"])
