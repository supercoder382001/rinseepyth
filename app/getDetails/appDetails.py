...
APP_DETAILS_URL = f"{SUPABASE_URL}/rest/v1/rpc/appdetails"

# Initialize FastAPI Router
router = APIRouter()


def fetch_app_details(authtokens):
    ...
    response = requests.get(APP_DETAILS_URL, headers=headers)
    ...
    raise HTTPException(status_code=404, detail="No app details found")


@router.get("/appdetails")
async def app_details(authtoken: str = Header(...)):
    """Decrypts authtoken and fetches app details from Supabase."""
    try:
    ...
    supabase_response = fetch_app_details(authtokens["details"])
